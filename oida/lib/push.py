#!/usr/bin/env python3
"""OIDA for Claude — build + push SessionEnvelopes (deterministic, no LLM).

For each planned session: assemble the SessionEnvelope (the exact wire contract
the server validates — session_id, surface, repo, started/ended_at, update,
transcript{turns, tool_calls, metrics, git_stats}, client_version, git_email),
...
CLIENT_VERSION = "oida-client/0.1.0"
SURFACE = "claude_code"
MAX_ATTEMPTS = 4
...
def build_envelope(desc):
    ...
    transcript was already redacted by transcript.py."""
