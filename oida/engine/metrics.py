#!/usr/bin/env python3
"""OIDA for Claude — deterministic session metrics.

Ported from control8's `session_digest.py`: git line stats for the session's
commit window, active (hands-on) time with capped idle gaps, and commit messages.
All pure/defensive — a missing git, a non-repo dir, or an unparseable timestamp
degrades to empty, never raises. These land in the envelope's `transcript.metrics`
and `transcript.git_stats` (quarantined in `events.payload` server-side).
"""
import os
import subprocess
import sys
